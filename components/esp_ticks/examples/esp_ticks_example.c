...
app_main(void) {
    ...
    timer_context_t periodic_ctx = {.counter = 0, .name = "Periodic-1s"};
    tick_handle_t periodic_timer;
    ESP_ERROR_CHECK(ticks_create(&periodic_timer, periodic_timer_callback, TICK_PERIODIC, &periodic_ctx));
    ...
    ESP_ERROR_CHECK(ticks_create(&oneshot_timer, oneshot_timer_callback, TICK_ONCE, (void*)"5 seconds elapsed!"));
    ...
    ESP_ERROR_CHECK(ticks_create(&led_timer, led_blink_callback, TICK_PERIODIC, NULL));
    ...
    for (int i = 0; i < 3; i++) {
        ...
        ESP_ERROR_CHECK(ticks_create(&multi_timers[i], periodic_timer_callback, TICK_PERIODIC, &multi_ctx[i]));
